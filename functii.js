...
console.log(getOddsSum(5));
console.log(getOddsSum(8));
...
console.log(getEvenSum(1,4));
console.log(getEvenSum(6,2));

// Объект Math
const a = Math.abs(7); // 7 Возвращает абсолютное значение числа.
const b = Math.abs(-7); // 7

const c = Math.min(2, 19, 3, -10, 9, 84); // -10
const d = Math.max(2, 19, 3, -10, 9, 84); // 84

const e = Math.pow(2, 4); // 2 ** 4 = 16 //Возвращает a в n-й степени, то есть an.

const f = Math.floor(4.99); // 4 (вниз)
const g = Math.ceil(4.000001); // 5 (вверх)
const h = Math.round(4.32); // 4 (по правила математики)Возвращает значение числа, округлённое до ближайшего целого
//Math - это встроенный объект в JavaScript, который предоставляет набор методов и свойств для выполнения математических операций и вычислений. С помощью объекта 
...
console.log(getEvenSum1(1,4));
console.log(getEvenSum1(6,2));

// 4. Сгенерировать случайное целое число от 0 до 10
const step_1 = Math.random();// случайное дробное число от 0 до 1
const step_2 = Math.step_1 * 10;// случайное дробное число от 0 до 10
