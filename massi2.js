...
// .reduce()
// .sort()
//rabota s tiklom eto deklarotivnii podhod
// arr.forEach(num => new_arr.push(num * 2));-eto iteractivnii podhod
// 1. Написать функцию, принимающую массив с числами и вовзаращающую новый массив с этими числами умноженными
...
const multNumbers2 = arr=>{
    const new_arr = [];
    arr.forEach(num => new_arr.push(num * 2));//mojno pisati elem =element nazivaem vizivaem element.forEach eto metod massiva v kotoruiu peraddaiom functiiu
    return new_arr
}
const multNumber3 = arr =>arr.map(el =>el * 2);//metod map
console.log(multNumber2([ 3, 5, 10,14]));
...
const changeStrings = arr => {
   ...
   console.log(changeStrings([4, 'hello', true, 12,  'bye']));
